...
public class Command implements Constants{
	...
	public static void upload() throws IOException {
		//element.sendKeys("Path"); Edit Text Path u can type
		driver.pushFile("/mobiledevicepath/path", new File("/systempath/x/y"));
		//gallery 
	...
	public static AndroidElement getXpathContentWise(String expression) {
		...
		// Xpath Variety
		//*[@AttributeName=AttributeValue] (Access All Tags)
		//android.widget.Layout[index]
		//android.widget.Layout/android.widget.TextView[2]   (Nested)
		//*[@text='Amit'] (Lookup for Amit Attribute Value
		//*[contains(@text,'A')]
		//*[@text='Amit' or @label='Ram']
		
		
	}
	
	public static void click(String type, String expression, int index) {
		 driver.findElementsByAndroidUIAutomator(expression).get(index).click();
	}
	...
	public static int assertCountTrue(String type, String expression, Object value) {
		...
		tempvalue = ((int)Double.parseDouble(value.toString()));
		}
		catch(Exception e) {
			tempvalue = -1;
		}
		...
		//return (elements.size() == tempvalue);
	...
	/**
	 * Open is a method use to open apk and set the DesiredCapabilites 
	 * @return
	 * @throws MalformedURLException
	 */
	public static AndroidDriver<AndroidElement> open(String apkPath) throws MalformedURLException{
		String serverURL = PropertyReader.getValue(SERVER_URL_KEY);
		String apkLocation = apkPath;
	...
	public static void waitForClickable(AndroidElement ele) {
		final int MAX = 10;
		WebDriverWait waitObj = new WebDriverWait(driver, MAX);
		waitObj.until(ExpectedConditions.elementToBeClickable(ele));
	}
	
	public static void waitForVisible(AndroidElement ele) {
		final int MAX = 10;
		WebDriverWait waitObj = new WebDriverWait(driver, MAX);
		waitObj.until(ExpectedConditions.visibilityOf(ele));
	}
	
	public static void nestedElementClick(String type, String expression, AndroidElement parentElement) {
		parentElement.findElement(By.xpath(expression)).click();
	}
	
	
	public static AndroidElement getElement(String type, String expression) {
...
//		else
	...
	public static void clickByIndex(String type, String expression, int index) {
		List<AndroidElement> elements = getElements(type, expression);
